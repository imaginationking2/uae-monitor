...
const results = {
  ...
  dubizzle: null, dubizzle_motors: null,
...
// Dubizzle targets — will run WITHOUT proxy (direct connection)
...
// Non-Dubizzle targets — use proxy
const OTHER_TARGETS = [
...
function parseDubizzlePropertySale(text, title) {
  const m = (title || '').match(/([\d,]+)\s+Properties for Sale/i);
...
function parseLuxury(title, text) {
  ...
  return { drop_count: count, max_drop_pct: maxDrop, avg_drop_pct: 6.5, off_market_deals: null };
}
...
async function handleRequest({ request, page, log }) {
  ...
  // Extra wait for Dubizzle React pages to render
  if (id.startsWith('dubizzle')) {
    await sleep(2000);
  }
  ...
  const bodyText = await page.evaluate(() => {
    ...
    return inner.trim().length > 100 ? inner : '';
  });
  ...
  switch (id) {
    ...
    case 'dubizzle_property_sale':
      ...
      results.dubizzle_property_sale = parsed; break;
    ...
    case 'bayut_ajman_rent':
      ...
      results.bayut_ajman_rent = parsed; break;
    case 'benchmark':
...
// ── Crawler 1: Dubizzle — NO proxy (direct connection bypasses bot detection) ─
...
// ── Crawler 2: Others — use residential proxy ─────────────────────────────────
let proxyConfiguration;
try {
  proxyConfiguration = await Actor.createProxyConfiguration({ groups: ['RESIDENTIAL'], countryCode: 'AE' });
} catch (e) {
  console.log('Proxy config failed: ' + e.message);
}

const otherCrawler = new PlaywrightCrawler({
  proxyConfiguration,
  maxRequestRetries: 3,
...
// Run Dubizzle first (no proxy), then others (with proxy)
await dubizzleCrawler.run(DUBIZZLE_TARGETS.map(t => ({ url: t.url, userData: { id: t.id } })));
await otherCrawler.run(OTHER_TARGETS.map(t => ({ url: t.url, userData: { id: t.id } })));
